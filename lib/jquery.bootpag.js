...
(function($, window) {
    ...
    $.fn.bootpag = function(options){
	    ...
	    if(!$currPage.not('.next,.previous').length){
                var d = page <= vis ? -settings.maxVisible : 0;
		$page.not('.next,.previous').each(function(index){
                    lp = index + 1 + vis + d;
                    $(this)
                        .attr('data-lp', lp)
            ...
            $currPage.not('.next,.previous').addClass('active');
		...
		p.push('<li data-lp="1" class="previous"><a href="'+href(1)+'">'+settings.previous+'</a></li>');
            }
